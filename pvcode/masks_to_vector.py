...
#处理屋顶分割和方向分析的 Python 模块，主要用于从图像掩码中提取屋顶区域、简化几何形状、计算朝向并处理重叠区域

#将栅格格式的掩码图像转换为矢量格式的几何图形，并以 GeoDataFrame 的形式返回
def get_vector_labels(file_path, mask_filename, gdf_images, label_classes, area_treshold, bg_is_0=True):
    ...
    print('bbb:',mask_filename)
    print(list(gdf_images.id))
...
#功能是简化屋顶分割的几何形状并计算每个分割区域的方位角（azimuth）
def segment_simplify_and_add_azimuth(gdf_segments, visualize=True):#visualize：布尔值，是否可视化结果（默认不可视化）
    ...
    gdf_segments = gdf_segments.set_geometry('geometry')  # 处理RID2时添加这行
