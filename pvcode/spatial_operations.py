...
def find_longest_line(polygon):
    """
    Finds longest line in roof segment to align solar panels.# 寻找屋顶片段中最长的线段，用于太阳能面板对齐
    """
...
#获取目录中 GeoTIFF 图像的 GeoDataFrame
...
#一套地理空间数据处理工具，主要功能包括：
# 多边形分析（最长线段提取、重叠过滤、简化）；
# 方位角与标签的相互转换；
# 图像与地理数据的转换（栅格转矢量、坐标映射）；
# 地理栅格文件（GeoTIFF）的读取与处理。