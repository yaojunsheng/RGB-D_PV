...
class RGBStyleEvolution(nn.Module):
    def __init__(self, feature_channels=18, num_styles=8, style_dim=32):
        ...
        self.semantic_encoder = nn.ModuleDict({
            'texture': nn.Sequential(nn.Conv2d(feature_channels, style_dim//4, 3, padding=1), nn.BatchNorm2d(style_dim//4), nn.ReLU(), nn.AdaptiveAvgPool2d(1), nn.Flatten()),
            'color': nn.Sequential(nn.Conv2d(feature_channels, style_dim//4, 5, padding=2), nn.BatchNorm2d(style_dim//4), nn.ReLU(), nn.AdaptiveAvgPool2d(2), nn.Flatten(), nn.Linear(style_dim//4 * 4, style_dim//4)),
            'appearance': nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(feature_channels, style_dim//4)),
            'semantic': nn.Sequential(nn.Conv2d(feature_channels, style_dim//4, 1), nn.AdaptiveAvgPool2d(1), nn.Flatten())
        })
        self.semantic_fusion = nn.Sequential(nn.Linear(style_dim, style_dim), nn.ReLU(), nn.Linear(style_dim, style_dim), nn.LayerNorm(style_dim))
        self.visual_modulator = nn.Sequential(nn.Linear(style_dim + 6, feature_channels), nn.Tanh())
...
class DepthGeometricEncoder(nn.Module):
    def __init__(self, feature_channels=18):
        ...
        self.geometric_encoders = nn.ModuleList([
            nn.Sequential(nn.Conv2d(1, feature_channels // 4, 3, padding=1), nn.BatchNorm2d(feature_channels // 4), nn.ReLU()),
            nn.Sequential(nn.Conv2d(1, feature_channels // 4, 5, padding=2), nn.BatchNorm2d(feature_channels // 4), nn.ReLU()),
            nn.Sequential(nn.Conv2d(1, feature_channels // 4, 7, padding=3), nn.BatchNorm2d(feature_channels // 4), nn.ReLU())
        ])
        self.gradient_processor = nn.Sequential(nn.Conv2d(3, feature_channels // 4, 3, padding=1), nn.BatchNorm2d(feature_channels // 4), nn.ReLU())
        self.geometric_fusion = nn.Sequential(nn.Conv2d(16, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 1), nn.BatchNorm2d(feature_channels), nn.ReLU())
        self.boundary_enhancer = nn.Sequential(nn.Conv2d(feature_channels, feature_channels // 2, 3, padding=1), nn.BatchNorm2d(feature_channels // 2), nn.ReLU(), nn.Conv2d(feature_channels // 2, feature_channels // 2, 3, padding=2, dilation=2), nn.BatchNorm2d(feature_channels // 2), nn.ReLU(), nn.Conv2d(feature_channels // 2, 1, 1), nn.Sigmoid())
...
class DepthSpatialReasoner(nn.Module):
    def __init__(self, feature_channels=18):
        super().__init__()
        self.spatial_conv = nn.Sequential(nn.Conv2d(feature_channels, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 3, padding=2, dilation=2), nn.BatchNorm2d(feature_channels), nn.ReLU())
        self.multi_scale_reasoning = nn.ModuleList([nn.Sequential(nn.AdaptiveAvgPool2d(size), nn.Conv2d(feature_channels, feature_channels // 4, 1), nn.ReLU(), nn.Upsample(scale_factor=1, mode='bilinear', align_corners=True)) for size in [(1, 1), (2, 2), (4, 4)]])
        self.reasoning_fusion = nn.Sequential(nn.Conv2d(30, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 1), nn.BatchNorm2d(feature_channels), nn.Sigmoid())
...
class LocalHeightAttention(nn.Module):
    def __init__(self, channels, kernel_size=7):
        ...
        self.multi_scale_conv = nn.ModuleList([nn.Conv2d(1, channels // 8, 3, padding=1), nn.Conv2d(1, channels // 8, 5, padding=2), nn.Conv2d(1, channels // 8, 7, padding=3)])
        self.gradient_conv = nn.Sequential(nn.Conv2d(1, channels // 8, 3, padding=1), nn.BatchNorm2d(channels // 8), nn.ReLU())
        self.edge_enhance = nn.Sequential(nn.Conv2d(1, channels // 8, 3, padding=1), nn.BatchNorm2d(channels // 8), nn.ReLU(), nn.Conv2d(channels // 8, channels // 8, 3, padding=2, dilation=2), nn.BatchNorm2d(channels // 8), nn.Sigmoid())
        self.refine_base = nn.Sequential(nn.Conv2d(channels // 2, channels, 3, padding=1), nn.BatchNorm2d(channels), nn.ReLU(), nn.Conv2d(channels, channels, 3, padding=1, groups=channels//4), nn.BatchNorm2d(channels), nn.ReLU())
        self.refine_edge = nn.Sequential(nn.Conv2d(channels // 8, channels // 4, 1), nn.BatchNorm2d(channels // 4), nn.ReLU())
        self.final_fusion = nn.Sequential(nn.Conv2d(channels + channels // 4, channels, 3, padding=1), nn.BatchNorm2d(channels), nn.ReLU())
        self.attention_gen = nn.Sequential(nn.Conv2d(channels, channels // 4, 1), nn.ReLU(), nn.Conv2d(channels // 4, channels, 1), nn.Sigmoid())
...
class UnifiedHeightEncoder(nn.Module):
    def __init__(self, input_channels=1, prompt_channels=16):
        ...
        self.shared_conv = nn.Sequential(nn.Conv2d(input_channels, 32, 3, 1, 1), nn.BatchNorm2d(32), nn.ReLU(), nn.Conv2d(32, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.unified_branch = nn.Sequential(nn.Conv2d(64, prompt_channels, 5, 1, 2), nn.BatchNorm2d(prompt_channels), nn.ReLU(), nn.Conv2d(prompt_channels, prompt_channels, 3, 1, 1), nn.BatchNorm2d(prompt_channels), nn.ReLU(), nn.Conv2d(prompt_channels, prompt_channels, 3, 1, 1), nn.BatchNorm2d(prompt_channels), nn.ReLU(), nn.Conv2d(prompt_channels, prompt_channels, 3, 1, 2, dilation=2), nn.BatchNorm2d(prompt_channels), nn.ReLU(), nn.Conv2d(prompt_channels, prompt_channels, 1, 1, 0), nn.BatchNorm2d(prompt_channels), nn.ReLU())
        self.local_height_attention = LocalHeightAttention(prompt_channels, kernel_size=5)
        self.fine_detail_detector = nn.Sequential(nn.Conv2d(1, 16, 3, padding=1), nn.BatchNorm2d(16), nn.ReLU(), nn.Conv2d(16, 16, 3, padding=2, dilation=2), nn.BatchNorm2d(16), nn.ReLU(), nn.Conv2d(16, prompt_channels // 2, 1), nn.Sigmoid())
        self.feature_fusion = nn.Sequential(nn.Conv2d(prompt_channels + prompt_channels // 2, prompt_channels, 3, padding=1), nn.BatchNorm2d(prompt_channels), nn.ReLU(), nn.Conv2d(prompt_channels, prompt_channels, 1), nn.BatchNorm2d(prompt_channels))
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.global_fc = nn.Sequential(nn.Linear(prompt_channels, prompt_channels // 4), nn.ReLU(), nn.Linear(prompt_channels // 4, prompt_channels), nn.Sigmoid())
...
class UnifiedPrompt(nn.Module):
    def __init__(self, feature_channels, prompt_channels=64):
        ...
        self.prompt_channels = prompt_channels
        self.channel_prompt = nn.Sequential(nn.Conv2d(prompt_channels, feature_channels, 1), nn.Sigmoid())
        self.spatial_prompt = nn.Sequential(nn.Conv2d(prompt_channels, 1, 3, padding=1), nn.Sigmoid())
        self.point_wise_prompt = nn.Sequential(nn.Conv2d(prompt_channels, feature_channels, 1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 1), nn.Sigmoid())
        self.residual_enhance = nn.Sequential(nn.Conv2d(feature_channels, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 1), nn.Sigmoid())
        self.fusion_conv = nn.Sequential(nn.Conv2d(feature_channels + prompt_channels, feature_channels * 2, 3, 1, 1), nn.BatchNorm2d(feature_channels * 2), nn.ReLU(), nn.Conv2d(feature_channels * 2, feature_channels, 1), nn.BatchNorm2d(feature_channels), nn.ReLU())
...
class UnifiedMultiScaleHeightPrompt(nn.Module):
    def __init__(self, branch_channels=[18, 36, 72, 144], prompt_channels=64):
        ...
        self.height_encoder = UnifiedHeightEncoder(input_channels=1, prompt_channels=prompt_channels)
        self.unified_prompts = nn.ModuleList([UnifiedPrompt(branch_channels[i], prompt_channels) for i in range(self.num_branches)])
        self.unified_weight = nn.Parameter(torch.tensor(0.6))
        self.feature_recalibration = nn.ModuleList([nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(branch_channels[i], branch_channels[i] // 4, 1), nn.ReLU(), nn.Conv2d(branch_channels[i] // 4, branch_channels[i], 1), nn.Sigmoid()) for i in range(self.num_branches)])
        self.self_guided_attention = nn.ModuleList([nn.Sequential(nn.Conv2d(branch_channels[i], branch_channels[i] // 2, 3, padding=1), nn.BatchNorm2d(branch_channels[i] // 2), nn.ReLU(), nn.Conv2d(branch_channels[i] // 2, 1, 1), nn.Sigmoid()) for i in range(self.num_branches)])
    def forward(self, multi_scale_features, height_map):
        ...
        for branch_idx, features in enumerate(multi_scale_features):
            enhanced_feat = self.unified_prompts[branch_idx](features, height_features, global_prompt)
...
class CrossModalInnovativeFusion(nn.Module):
    def __init__(self, feature_channels=18):
        super().__init__()
        self.modal_alignment = nn.ModuleDict({'rgb': nn.Sequential(nn.Conv2d(feature_channels, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU()), 'depth': nn.Sequential(nn.Conv2d(feature_channels, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU())})
        self.cross_fusion = nn.Sequential(nn.Conv2d(feature_channels * 2, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.ReLU(), nn.Conv2d(feature_channels, feature_channels, 1), nn.BatchNorm2d(feature_channels), nn.ReLU())
        self.adaptive_weight_gen = nn.Sequential(nn.Linear(feature_channels * 4, feature_channels * 2), nn.ReLU(), nn.Linear(feature_channels * 2, 3), nn.Softmax(dim=1))
        self.feature_enhancer = nn.Sequential(nn.Conv2d(feature_channels * 2, feature_channels * 2, 3, padding=1), nn.BatchNorm2d(feature_channels * 2), nn.ReLU(), nn.Conv2d(feature_channels * 2, feature_channels, 1), nn.BatchNorm2d(feature_channels), nn.ReLU())
        self.cross_residual = nn.Sequential(nn.Conv2d(feature_channels, feature_channels, 3, padding=1), nn.BatchNorm2d(feature_channels), nn.Sigmoid())
    def forward(self, rgb_features, depth_features):
        ...
        return final_features, {'fusion_weights': fusion_weights, 'cross_modal_similarity': F.cosine_similarity(rgb_global, depth_global, dim=1).mean().item()}
...
class DualBranchInnovativeRGBD(nn.Module):
    def __init__(self, num_classes=9, backbone_name='hrnet_w18', enable_chfi=True):
        ...
        if self.enable_chfi:
            ...
            self.gse_adapter_layer1 = nn.Sequential(nn.Conv2d(18, 36, 1), nn.BatchNorm2d(36), nn.ReLU())
            self.depth_spatial_reasoner = DepthSpatialReasoner(feature_channels=18)
            self.unified_height_prompt = UnifiedMultiScaleHeightPrompt(branch_channels=backbone_channels, prompt_channels=64)
        if self.enable_chfi:
            self.cross_modal_fusion = nn.ModuleList([CrossModalInnovativeFusion(feature_channels=ch) for ch in backbone_channels])
        else:
            self.cross_modal_fusion = nn.ModuleList([nn.Sequential(nn.Conv2d(ch * 2, ch, 3, padding=1), nn.BatchNorm2d(ch), nn.ReLU()) for ch in backbone_channels])
    ...
    def forward(self, x, masks=None, task_id=None, mode='train'):
        ...
        if self.enable_chfi:
            ...
            target_size_0 = depth_reasoned_0.shape[2:]
            if depth_geometric_0.shape[2:] != target_size_0:
                gse_0 = F.interpolate(depth_geometric_0, size=target_size_0, mode='bilinear', align_corners=True)
            else:
                gse_0 = depth_geometric_0
            fused_depth_0 = 0.2 * gse_0 + 0.6 * depth_reasoned_0 + 0.2 * depth_prompted_features[0]
            target_size_1 = depth_prompted_features[1].shape[2:]
            gse_1 = F.interpolate(depth_geometric_0, size=target_size_1, mode='bilinear', align_corners=True)
            gse_1 = self.gse_adapter_layer1(gse_1)
            fused_depth_1 = 0.4 * gse_1 + 0.6 * depth_prompted_features[1]
            depth_features = [fused_depth_0, fused_depth_1, depth_prompted_features[2], depth_prompted_features[3]]
        fused_features = []
        fusion_stats = []
        ...
        if hasattr(self, 'continual_enhancer') and self._cl_enabled:
            enhanced_feat_0, cl_info = self.continual_enhancer(fused_features[0], None, task_id, enable_update=(mode == 'train'))
        ...
        self._fusion_stats = {'rgb_style': rgb_style_info, 'fusion_stats': fusion_stats, 'continual_learning': cl_info}
        return final_output,final_feat

class ContinualLearningRoofSegmentationMTL(nn.Module):
    def __init__(self, base_model_class=DualBranchInnovativeRGBD, enable_continual_learning=True, **base_model_kwargs):
        super().__init__()
        ...
        if enable_continual_learning:
            self.base_model = enhance_dual_branch_model_with_continual_learning(self.base_model, rgb_feature_channels=18, enable_fusion_pattern=True, enable_meta=False)
    def forward(self, x, height_map=None, mode='train'):
        if height_map is not None:
            rgbd_input = torch.cat([x, height_map], dim=1)
        else:
            zero_depth = torch.zeros(x.size(0), 1, x.size(2), x.size(3), device=x.device, dtype=x.dtype)
...
def get_roof_MTL(num_classes=9, backbone_name='hrnet_w18', enable_continual_learning=True, enable_chfi=True):
    model = ContinualLearningRoofSegmentationMTL(base_model_class=DualBranchInnovativeRGBD, enable_continual_learning=enable_continual_learning, enable_chfi=enable_chfi, num_classes=num_classes, backbone_name=backbone_name)
    return model
...
__all__ = ['get_roof_MTL', 'apply_delayed_updates_after_backward', 'clear_pending_updates']