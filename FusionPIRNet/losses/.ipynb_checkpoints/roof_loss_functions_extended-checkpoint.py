...
类名保持一致版本，可直接替换原有的ComputeRoofLossExtended
"""
import torch
...
class ComputeRoofLossExtended(ComputeRoofLoss):
    ...
    def compute_supervision(self, x_pred_seg6, x_output_seg6, x_pred_seg9, x_output_seg9, 
        ...
        # # height loss: l1 norm with binary mask (参考开源版本)
        # # binary mark to mask out undefined pixel space
        # binary_mask = (torch.sum(x_output_height, dim=1) != 0).type(torch.FloatTensor).unsqueeze(1).cuda()
        # loss_height = torch.sum(torch.abs(x_pred_height - x_output_height) * binary_mask) / torch.nonzero(binary_mask).size(0)
        # 修改：使用简单的L1损失，因为数据已经归一化
        # binary_mask = (x_output_height != 0).type(torch.FloatTensor).cuda()  # 修改mask条件
        # loss_height = torch.sum(torch.abs(x_pred_height - x_output_height) * binary_mask) / (torch.sum(binary_mask) + 1e-6)
        # height loss: simple L1 loss, since height labels already normalized
        loss_height = F.l1_loss(x_pred_height, x_output_height)
        # roof_mask = (x_output_height > 0.1).float()  # 排除0.1以下的地面区域
        # masked_loss = F.l1_loss(x_pred_height * roof_mask, x_output_height * roof_mask, reduction='sum')
        # loss_height = masked_loss / (roof_mask.sum() + 1e-6)
    ...
    def compute_distill_loss(self, s_pred_seg6, t_pred_seg6, s_pred_seg9, t_pred_seg9,
        ...
        # height distillation loss (参考开源版本的depth蒸馏方法)
